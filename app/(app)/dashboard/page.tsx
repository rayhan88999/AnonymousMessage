...
const page = () => {
    ...
    const fetchAcceptMessage= useCallback(async()=>{
         ...
         try {
         const responsne= await axios.get<ApiResponse>("/api/accept-messages")
         setValue("acceptMessage",responsne.data.isAcceptingMessage ?? false)
         } catch (error) {
  ...
  const copyToClipboard = () => {
    ...
    <div className="my-8 mx-4 md:mx-8 lg:mx-auto p-6 bg-white rounded w-full max-w-6xl">
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        { isLoading? (
          [...Array(4)].map((_,i)=> <MessageCardSkeleton key={i}/>)
        ):messages.length > 0 ? (
